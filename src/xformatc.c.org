...
static int name##2(type val,int prec,char * out) \
{ \
	int length = 0; \
	while (prec -- > 0 || val) \
	{ \
		*out-- = ms_digits[val & 0x01]; \
		val   >>= 1; \
		length ++; \
	} \
	return length; \
} \
static int name##8(type val,int prec,char *out) \
{ \
	int length = 0; \
	while (prec -- > 0 || val) \
	{ \
		*out -- = ms_digits[val & 0x07]; \
		val     >>= 3; \
		length++; \
	} \
	return length; \
} \
static int name##10(type val,int prec,char *out) \
{ \
	char * org = out; \
	while (prec -- > 0 || val) \
	{ \
		*out -- = ms_digits[val % 10]; \
		val   /= 10; \
	} \
	return org - out; \
} \
static int name##16(type val,int prec,char *out) \
{ \
	int length = 0; \
	while (prec -- > 0 || val) \
	{ \
		*out -- = ms_digits[val & 0x0F]; \
		val   >>=  4; \
		length++; \
	} \
	return length; \
...
{ \
	...
	{ \
		case	2: \
			length = name##2(param->values.value,param->prec,param->out); \
			break; \
		case	8: \
			length = name##8(param->values.value,param->prec,param->out); \
			break; \
		default: \
		case	10: \
			length = name##10(param->values.value,param->prec,param->out); \
			break; \
		case	16: \
			length = name##16(param->values.value,param->prec,param->out); \
			break; \
	} \
	param->length += length; \
	param->out -= length; \
...
#else
U2A(ullong2a,unsigned long long,llvalue)
#endif
#endif
...
//							param.values.dvalue /= 10.0;
	...
	//					param.values.lvalue = (unsigned LONG)param.dbl;
						param.values.lvalue = (unsigned LONG)(param.dbl * xpow10(param.prec));
